#!/usr/bin/env python3
"""
Phase 1.5: Detect Contradictions (AMPAYs) - Model CLI Version
Compares 2021 promises against actual votes to find contradictions.
Uses the configured language-model command.

Usage:
  python detect_ampays_model_cli.py --sample            # Test with 10 promises
  python detect_ampays_model_cli.py --all               # Run all promises
  python detect_ampays_model_cli.py --party peru_libre  # Run for specific party
"""
...
# Configuration
MODEL_COMMAND_ENV = "AMPAY_MODEL_COMMAND"
...
def build_model_command(prompt: str) -> list[str]:
    """Build the configured model command for this prompt."""
    command_template = os.environ.get(MODEL_COMMAND_ENV)
    if not command_template:
        raise RuntimeError(
            f"Set {MODEL_COMMAND_ENV} to a shell-style command template containing '{{prompt}}'."
        )

    command = [part.format(prompt=prompt) for part in shlex.split(command_template)]
    if not command:
        raise RuntimeError(f"{MODEL_COMMAND_ENV} is empty")
    return command


def call_model(prompt, max_retries=3):
    """Call the configured model CLI to analyze promise vs votes."""
    for attempt in range(max_retries):
        try:
            result = subprocess.run(
                build_model_command(prompt),
                capture_output=True,
                text=True,
                timeout=120
            )
